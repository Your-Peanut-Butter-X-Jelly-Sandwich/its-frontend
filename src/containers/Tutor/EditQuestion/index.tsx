...
const EditQuestionContainer: React.FC<PropsType> = ({ qn_id }) => {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [qn_id, baseUrl, authToken]); 
  

  console.log(questionData);

  
  const [markdown, setMarkdown] = useState(questionData?.question?.question_statement);
  const [testCases, setTestCases] = useState([{ input: '', expectedOutput: '' }]);
  const [currentMode, setCurrentMode] = useState('markdown'); 
  const [language, setLanguage] = useState(questionData?.question?.language);
  const [codeContent, setCodeContent] = useState(questionData?.question?.ref_program);
  const [questionTitle, setQuestionTitle] = useState(questionData?.question?.question_title); 
  const [dueDate, setDueDate] = useState(questionData?.question?.due_date); 
